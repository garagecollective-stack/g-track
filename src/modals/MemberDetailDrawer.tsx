...
function TodoReadOnlyRow({ todo }: { todo: any }) {
  ...
  return (
    <div className={`px-4 py-3 border-b border-gray-50 last:border-0 ${done ? 'opacity-60' : ''}`}>
      <div className="flex items-start gap-3">
        <div className={`mt-0.5 w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 ${
          done ? 'bg-[#0A5540] border-[#0A5540]' : 'border-gray-300'
        }`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          {todo.due_date && (
            <p className={`flex items-center gap-1 text-xs mt-1 ${overdue ? 'text-red-500 font-medium' : 'text-gray-400'}`}>
...
export function MemberDetailDrawer({ member, onClose, allMembers = [] }: Props) {
  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 bottom-0 z-50 w-full md:w-[420px] bg-white border-l border-gray-200 shadow-xl flex flex-col">
        {/* Header + tabs */}
          ...
          {activeTab === 'todos' && (
            <div>
              <div className="px-5 py-3 border-b border-gray-100">
                <p className="text-sm font-semibold text-gray-900">{member.name.split(' ')[0]}'s To-Dos</p>
                {!loadingTodos && (
              ...
              {loadingTodos ? (
                <div className="flex items-center justify-center py-12">
                  <div className="w-5 h-5 border-2 border-[#0A5540] border-t-transparent rounded-full animate-spin" />
                </div>
              ) : memberTodos.length === 0 ? (
                ...
                <>
                  {pendingTodos.length > 0 && (
                    <>
                      <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide px-5 pt-4 pb-2">
