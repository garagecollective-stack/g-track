...
export function TodoIssueModal({ todo, onClose, onSuccess }: Props) {
  ...
  const inputCls = 'w-full border border-[var(--line-1)] rounded-[var(--r-sm)] px-3 py-2 text-sm text-[var(--ink-900)] placeholder-[var(--ink-400)] focus:outline-none focus:border-[var(--primary)] focus:ring-2 focus:ring-[var(--primary)]/15 transition-all'
      ...
      <div className="bg-[var(--surface-1)] rounded-2xl shadow-xl w-full max-w-md">
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-[var(--line-1)]">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-bold text-[var(--ink-900)]">Raise Issue</h2>
          </div>
        ...
        {/* Todo reference */}
        <div className="px-5 py-3 bg-[var(--surface-2)] border-b border-[var(--line-1)]">
          <p className="text-xs text-[var(--ink-400)]">Raising issue on todo:</p>
          <p className="text-sm font-medium text-[var(--ink-700)] mt-0.5 truncate">{todo.title}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div>
            <label className="text-sm font-medium text-[var(--ink-700)] block mb-1.5">Priority</label>
          ...
          <p className="text-xs text-[var(--ink-400)]">Directors will be notified of this issue.</p>
          ...
          <div className="flex justify-end gap-3 pt-1">
            ...
            <button type="submit" disabled={loading || !title.trim() || !description.trim()}
              className="px-4 py-2 text-sm font-medium text-white bg-amber-500 rounded-[var(--r-sm)] hover:bg-amber-600 transition-colors disabled:opacity-50 disabled:pointer-events-none">
